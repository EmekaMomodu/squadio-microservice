...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    void updateLoginFlagWithId(@Param(value = "userId") Long userId, @Param(value = "loginFlag") Character loginFlag);

    @Transactional
    @Modifying
    @Query("update User user set user.loginFlag = :loginFlag where upper(user.username) = upper(:username)")
    void updateLoginFlagWithName(@Param(value = "username") String username, @Param(value = "loginFlag") Character loginFlag);
